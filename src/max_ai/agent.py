...
class MaxAgent:
    ...
    def __init__(
            ...
            try:
                ...
                if active_profile:
                    if not resolved_api_key and active_profile.get("api_key"):
                        resolved_api_key = str(active_profile["api_key"])
                    if not resolved_base_url and active_profile.get("base_url"):
                        resolved_base_url = str(active_profile["base_url"])
                    if not resolved_model and active_profile.get("model"):
                        resolved_model = str(active_profile["model"])
                    if active_profile.get("api_mode"):
                        resolved_api_mode = str(active_profile["api_mode"])
            except Exception:
                pass
        ...
        if enable_tools:
            ...
            if skills and self.skill_manager:
                self.system_prompt = self.skill_manager.compose_system_prompt(self.system_prompt, skills)
