...
class MaxDiscordBot:
    ...
    def run(self) -> None:
        ...
        async def on_ready():
            ...
            user_name = str(bot.user) if bot.user else "MAX Bot"
            print(f"🤖 MAX Discord Bot is online as {user_name} (ID: {user_id})")
        ...
        async def on_message(message: discord.Message):
            ...
            is_mention = (bot_user in message.mentions) if (self.respond_to_mentions and bot_user) else False
            ...
            elif is_mention and bot_user:
                prompt = content.replace(f"<@{bot_user.id}>", "").replace(f"<@!{bot_user.id}>", "").strip()
            ...
            if prompt.lower() in ("help", "!help"):
                bot_name = bot_user.name if bot_user else "MAX Bot"
                help_text = (
                    "**🤖 MAX AI Discord Bot**\n"
                    f"• โมเดลปัจจุบัน: `{self.agent.model}`\n"
                    f"• คำสั่งถาม: `{self.command_prefix}<คำถาม>` หรือ Mention @{bot_name}\n"
